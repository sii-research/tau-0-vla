...
def original_format_state_dict(model, state_dict):
    """Keep the architecture used to train the published robotics adapter.

    The v1p2 diffusers snapshot includes a separate text-token residual branch.
    This original-format network and the robotics trainer do not use that branch.
    Only its two known tensors may be omitted; all active weights load strictly.
    """
    if not isinstance(model, Step1XEdit) or model.params.version != "v1p2":
        return state_dict
    unused = {"text_token_mapping.weight": (4096, 3584), "text_token_mapping.bias": (4096,)}
    present = set(unused) & state_dict.keys()
    if not present:
        return state_dict
    if present != set(unused) or any(tuple(state_dict[k].shape) != shape for k, shape in unused.items()):
        raise ValueError("Incomplete or incompatible v1p2 text_token_mapping tensors")
    warnings.warn("Omitting the two v1p2 text_token_mapping tensors unused by the robotics architecture.")
    return {k: v for k, v in state_dict.items() if k not in unused}
