...
from modules.model_edit import Step1XParams, Step1XEdit, original_format_state_dict
...
def load_state_dict(model, ckpt_path, device="cuda", strict=True, assign=True):
    ...
    missing, unexpected = model.load_state_dict(
        original_format_state_dict(model, state_dict), strict=strict, assign=assign
    )
...
class ImageGenerator:
    ...
    def denoise(
        ...
        half_N = img.shape[0] // 2 if cfg_guidance != -1 else img.shape[0]
        ref_img_tensor = img[:half_N, img.shape[1] // 2:].clone()
        if self.offload:
            self.dit = self.dit.to(self.device)
        if show_progress:
            pbar = tqdm(itertools.pairwise(timesteps), desc='denoising...')
            ...
            pbar = itertools.pairwise(timesteps)
        for idx, (t_curr, t_prev) in enumerate(pbar):
            if img.shape[0] == half_N and cfg_guidance != -1:
                img = torch.cat([img, img], dim=0)
            t_vec = torch.full(
                (img.shape[0],), t_curr, dtype=img.dtype, device=img.device
            )
    ...
    def generate_image_batch(
        ...
        """True batch inference for multiple images at fixed resolution.

        Args:
            prompts: list of N prompt strings
            negative_prompt: single negative prompt (shared)
            ref_images: list of N PIL Images
            num_steps: diffusion steps
            cfg_guidance: classifier-free guidance scale
            seed: base seed (each image gets seed+i)
            size_level: target resolution level
            height/width: if provided, force all images to this size
            show_progress: show tqdm bar
        Returns:
            list of N PIL Images
        """
        N = len(prompts)
        assert N == len(ref_images), "prompts and ref_images must have same length"
        assert N >= 1
        ...
        for img in ref_images:
            resized, original_size = self.input_process_image(img, img_size=size_level)
            if img_info is None:
                img_info = original_size
                target_h, target_w = resized.height, resized.width
            else:
        ...
        if height is not None and width is not None:
            ...
            processed_images = [img.resize((target_w, target_h), Image.LANCZOS) for img in processed_images]

        ref_images_raw_tensors = torch.stack(
            [F.to_tensor(img.convert("RGB")) for img in processed_images]
        ).to(self.device)

        if self.offload:
            self.ae = self.ae.to(self.device)
        with torch.no_grad(), torch.cuda.amp.autocast(dtype=torch.bfloat16):
            ref_latents = self.ae.encode(ref_images_raw_tensors * 2 - 1)
        if self.offload:
            self.ae = self.ae.cpu()
            torch.cuda.empty_cache()
        ...
        ref_images_raw_cfg = torch.cat([ref_images_raw_tensors, ref_images_raw_tensors], dim=0)

        all_prompts = prompts + [negative_prompt] * N
        inputs = self.prepare(all_prompts, x_cfg, ref_image=ref_latents_cfg, ref_image_raw=ref_images_raw_cfg)
        ...
        x = self.unpack(x.float(), target_h, target_w)
        if self.offload:
            self.ae = self.ae.to(self.device)
        x = self.ae.decode(x)
        if self.offload:
        ...
        t1 = time.perf_counter()
        if os.getenv("TORCHELASTIC_RUN_ID") is None or dist.get_rank() == 0:
            print(f"Batch inference: {N} images in {t1 - t0:.1f}s ({(t1-t0)/N:.2f}s/img).")

        images_list = []
        for i in range(N):
            pil_img = F.to_pil_image(x[i].float())
            images_list.append(self.output_process_image(pil_img, ref_images[i].size))
        return images_list
