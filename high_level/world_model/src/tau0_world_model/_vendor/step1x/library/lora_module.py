...
class LoRANetwork(torch.nn.Module):
    ...
    def load_weights(self, file):
        ...
        info = self.load_state_dict(weights_sd, True)
        return info
    ...
    def merge_to(self, text_encoders, dit, weights_sd, dtype=None, device=None):
        ...
        expected = {
            f"{lora.lora_name}.{key}"
            for lora in self.text_encoder_loras + self.unet_loras
            for key in lora.state_dict() if not key.startswith("org_module.")
        }
        if set(weights_sd) != expected:
            raise ValueError(f"LoRA keys do not match the base model: missing={expected - set(weights_sd)}, unexpected={set(weights_sd) - expected}")
        for lora in self.text_encoder_loras + self.unet_loras:
            sd_for_lora = {}
            for key in weights_sd.keys():
