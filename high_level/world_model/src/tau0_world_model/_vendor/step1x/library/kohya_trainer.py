...
class NetworkTrainer:
        ...
        if len(text_encoder_conds) == 0 or text_encoder_conds[0] is None or train_text_encoder:
            ...
            with torch.set_grad_enabled(is_train and train_text_encoder), accelerator.autocast():
                ...
                if args.weighted_captions:
                    ...
                    if batch.get("input_ids_list") is not None:
                        input_ids = [ids.to(accelerator.device) for ids in batch["input_ids_list"]]
                    else:
                        ref_paths = batch.get("ref_image_paths")
                        if ref_paths and ref_paths[0] is not None:
                            from PIL import Image as _Image
                            ref_imgs = [_Image.open(p).convert("RGB") for p in ref_paths]
                            input_ids = tokenize_strategy.tokenize(batch["captions"], ref_imgs)
                        else:
                            input_ids = tokenize_strategy.tokenize(batch["captions"])
    ...
    def train(self, args):
        ...
        for i, t_enc in enumerate(text_encoders):
            ...
            t_dev = t_enc.device if isinstance(t_enc.device, torch.device) else torch.device(t_enc.device)
            if t_dev.type != "cpu":
