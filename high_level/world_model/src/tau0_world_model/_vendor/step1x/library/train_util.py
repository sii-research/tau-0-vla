    ...
    def __getitem__(self, index):
        ...
        for image_key in bucket[image_index : image_index + bucket_batch_size]:
            ...
            if image_info.latents is not None:  # cache_latents=Trueの場合
                ...
                image = None
                ref_image = None
                ref_latents = image_info.latents  # fallback; in-memory cache path rarely used with edit dataset
                ref_alpha_mask = None
                ...
                # load reference image (for edit dataset, no-cache path)
                ref_latents = None
                ref_alpha_mask = None
                if hasattr(image_info, 'ref_absolute_path') and image_info.ref_absolute_path is not None:
                    ref_img, _, _, _, _ = self.load_image_with_face_info(
                        subset, image_info.ref_absolute_path, False
                    )
                    if self.enable_bucket:
                        ref_img, _, _ = trim_and_resize_if_required(
                            subset.random_crop, ref_img, image_info.bucket_reso, image_info.resized_size,
                            resize_interpolation=image_info.resize_interpolation
                        )
                    else:
                        ref_h, ref_w = ref_img.shape[0:2]
                        if ref_h > self.height or ref_w > self.width:
                            ref_img = ref_img[: self.height, : self.width]
                    if flipped:
                        ref_img = ref_img[:, ::-1, :].copy()
                    ref_img = ref_img[:, :, :3]
                    ref_image = self.image_transforms(ref_img)
                    del ref_img
                else:
                    ref_image = None
            ...
            ref_image_paths.append(getattr(image_info, 'ref_absolute_path', None))
        ...
        example["ref_image_paths"] = ref_image_paths
    ...
    def __init__(
        ...
        for subset in subsets:
            ...
            for image_key, img_md in metadata.items():
                ...
                image_info = ImageInfo(image_key, subset.num_repeats, caption, False, abs_path)
                res = img_md.get("train_resolution")
                image_info.image_size = (res[1], res[0]) if res and len(res) == 2 else res
